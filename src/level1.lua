...
-- NEU: Pixelstern-Funktion ohne "g" (feste Größe von 4x4 Pixeln pro Block)
function pixelstern(x, y)
    love.graphics.rectangle("fill", x, y - 8, 4, 4)
    love.graphics.rectangle("fill", x - 8, y - 4, 20, 4)
    love.graphics.rectangle("fill", x - 4, y, 12, 4)
    love.graphics.rectangle("fill", x - 8, y + 4, 4, 4)
    love.graphics.rectangle("fill", x + 8, y + 4, 4, 4)
end
...
function level1.update(dt)
    ...
    -- Auf Plattformen landen
    for i = 1, #level1.plattformen do
         p = level1.plattformen[i]
...
function level1.draw()
    -- Hintergrund färben
  love.graphics.setBackgroundColor(0.03, 0.01, 0.08)
    ...
    -- Gefahren zeichnen (Rot)
    love.graphics.setColor(1, 0, 0)
    for i = 1, #level1.gefahren do
        love.graphics.rectangle("fill", level1.gefahren[i].x, level1.gefahren[i].y, level1.gefahren[i].w, level1.gefahren[i].h)
    end

    if not level1.item.collected then
        love.graphics.setColor(1, 0.9, 0) -- Pinsel auf Gelb setzen
        pixelstern(level1.item.x + 12, level1.item.y + 12)
    end
    ...
    if globalStars >= 1 then
        love.graphics.setColor(1, 0.9, 0)     -- Gelb
    else
        love.graphics.setColor(0.3, 0.3, 0.3) -- Grau
    end
    pixelstern(1470, 30)

    -- Zweiter Stern
    if globalStars >= 2 then
        love.graphics.setColor(1, 0.9, 0)     -- Gelb
    else
        love.graphics.setColor(0.3, 0.3, 0.3) -- Grau
    end
